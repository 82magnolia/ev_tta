...
class CommonTrainer(MiniBatchTrainer):
    ...
    def init_scheduler(self, **kwargs):
        ...
        """
        self.scheduler_type = getattr(self.cfg, 'scheduler_type', 'reduce_plateau')
        if self.scheduler_type is not None:
            print(f"Using {self.scheduler_type} as scheduler")
        else:
            print("Using no schedulers")

        if self.scheduler_type == 'reduce_plateau':
            self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode=getattr(self.cfg, 'scheduler_mode', 'max'), 
            verbose=True, patience=getattr(self.cfg, 'scheduler_patience', 3), factor=getattr(self.cfg, 'scheduler_factor', 0.1))
        elif self.scheduler_type == 'exp_decay':
            self.scheduler = optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=self.cfg.scheduler_gamma)
        elif self.scheduler_type == 'step_decay':
            self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=self.cfg.scheduler_step_size, gamma=self.cfg.scheduler_gamma)
        elif self.scheduler_type == 'multi_step_decay':
            self.scheduler = optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=self.cfg.scheduler_milestones, gamma=self.cfg.scheduler_gamma)
        else:
            self.scheduler = None 
